...
public class Knight4Script : MonoBehaviour {
    ...
    private RayoScriptLevel2 rogueScript;
    ...
    void FixedUpdate()
    {
        ...
        rogueScript = Rogue.GetComponent<RayoScriptLevel2>();
        if (rogueScript.dead)
        {
    ...
    {
        if (reversePath && transform.position.y > target.position.y && !spotted)
        {
            return true;
        }
        else if (reversePath && transform.position.y < target.position.y)
        {
            return false;
        }
        if (!reversePath && transform.position.y < target.position.y && !spotted)
